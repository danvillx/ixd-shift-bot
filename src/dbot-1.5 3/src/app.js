...
// TZ_OFFSET_HOURS = your local offset from UTC. EDT=-4, EST=-5.
// Set this as a Railway env var. Defaults to -4 (Eastern Daylight Time).
const TZ_OFFSET = parseInt(process.env.TZ_OFFSET_HOURS || '-4', 10);
...
function parseScheduleTime(raw) {
  ...
  try {
    ...
    // Current UTC ms and local ms
    const nowUtcMs   = Date.now();
    const nowLocalMs = nowUtcMs + TZ_OFFSET * 3600000;
    const nowLocal   = new Date(nowLocalMs);

    // Build target in local time
    const targetLocal = new Date(nowLocalMs);
    targetLocal.setUTCHours(hours, mins, 0, 0);

    // Roll to next day if less than 2 min in the future
    if (targetLocal.getTime() - nowLocal.getTime() < 2 * 60 * 1000) {
      targetLocal.setUTCDate(targetLocal.getUTCDate() + 1);
    }

    // Convert local target back to UTC unix timestamp
    return Math.floor((targetLocal.getTime() - TZ_OFFSET * 3600000) / 1000);
  } catch (e) {
    console.error('parseScheduleTime error:', e);
    return null;
  }
}
    ...
    view: {
      ...
      blocks: [
        ...
        { type: 'input', block_id: 'schedule_time', optional: true,
          label: { type: 'plain_text', text: '⏰ Schedule Post (optional — leave blank to post now)' },
          element: { type: 'static_select', action_id: 'schedule_time_input',
            placeholder: { type: 'plain_text', text: 'Select a time...' },
            options: [
              { text: { type: 'plain_text', text: 'Post Now'        }, value: 'now'   },
              { text: { type: 'plain_text', text: '7:00 PM'         }, value: '19:00' },
              { text: { type: 'plain_text', text: '8:00 PM'         }, value: '20:00' },
              { text: { type: 'plain_text', text: '9:00 PM'         }, value: '21:00' },
              { text: { type: 'plain_text', text: '9:30 PM (Q1 end)' }, value: '21:30' },
              { text: { type: 'plain_text', text: '10:00 PM'        }, value: '22:00' },
              { text: { type: 'plain_text', text: '11:00 PM'        }, value: '23:00' },
              { text: { type: 'plain_text', text: '12:00 AM (Q2 end)' }, value: '00:00' },
              { text: { type: 'plain_text', text: '1:00 AM'         }, value: '01:00' },
              { text: { type: 'plain_text', text: '2:00 AM'         }, value: '02:00' },
              { text: { type: 'plain_text', text: '2:45 AM (Q3 end)' }, value: '02:45' },
              { text: { type: 'plain_text', text: '3:00 AM'         }, value: '03:00' },
              { text: { type: 'plain_text', text: '4:00 AM'         }, value: '04:00' },
              { text: { type: 'plain_text', text: '5:00 AM'         }, value: '05:00' },
              { text: { type: 'plain_text', text: '5:30 AM (Q4 end)' }, value: '05:30' },
            ] } },
      ],
    },
  });
...
app.view('qreport_modal', async ({ ack, body, view, client }) => {
  ...
  const scheduleVal = v.schedule_time?.schedule_time_input?.selected_option?.value || 'now';
  const postAt = (scheduleVal && scheduleVal !== 'now') ? parseScheduleTime(scheduleVal) : null;
  ...
  await client.views.open({
    ...
    view: {
      ...
      blocks: [
        ...
        { type: 'divider' },
        { type: 'input', block_id: 'sr_channel_1',
          label: { type: 'plain_text', text: '📢 Post To (Channel 1)' },
          element: { type: 'conversations_select', action_id: 'sr_channel_1_input',
            placeholder: { type: 'plain_text', text: 'Select channel' },
            filter: { include: ['public','private'], exclude_bot_users: true } } },
        { type: 'input', block_id: 'sr_channel_2', optional: true,
          label: { type: 'plain_text', text: '📢 Post To (Channel 2 — optional)' },
          element: { type: 'conversations_select', action_id: 'sr_channel_2_input',
            placeholder: { type: 'plain_text', text: 'Optional second channel' },
            filter: { include: ['public','private'], exclude_bot_users: true } } },
        { type: 'input', block_id: 'sr_schedule', optional: true,
          label: { type: 'plain_text', text: '⏰ Schedule Post (optional — leave blank to post now)' },
          element: { type: 'static_select', action_id: 'sr_schedule_input',
            placeholder: { type: 'plain_text', text: 'Select a time...' },
            options: [
              { text: { type: 'plain_text', text: 'Post Now'          }, value: 'now'   },
              { text: { type: 'plain_text', text: '5:00 AM'           }, value: '05:00' },
              { text: { type: 'plain_text', text: '5:30 AM (EOS)'     }, value: '05:30' },
              { text: { type: 'plain_text', text: '6:00 AM'           }, value: '06:00' },
              { text: { type: 'plain_text', text: '6:30 AM'           }, value: '06:30' },
              { text: { type: 'plain_text', text: '7:00 AM'           }, value: '07:00' },
              { text: { type: 'plain_text', text: '7:30 AM'           }, value: '07:30' },
              { text: { type: 'plain_text', text: '8:00 AM'           }, value: '08:00' },
            ] } },
      ],
    },
  });
...
app.view('shiftreport_modal', async ({ ack, body, view, client }) => {
  ...
  const srChannel1     = v.sr_channel_1.sr_channel_1_input.selected_conversation;
  const srChannel2     = v.sr_channel_2?.sr_channel_2_input?.selected_conversation || null;
  const srChannels     = [srChannel1, srChannel2].filter(Boolean);
  const srScheduleVal  = v.sr_schedule?.sr_schedule_input?.selected_option?.value || 'now';
  const srPostAt       = (srScheduleVal && srScheduleVal !== 'now') ? parseScheduleTime(srScheduleVal) : null;

  await Promise.all(srChannels.map(ch => client.chat.postMessage({ channel: ch, text: '⏳ Generating *Shift Handoff*... hang tight!' })));
...
- NEVER say "elevated", "environment", "kickoff", "carry this momentum", "required intervention"`;
    ...
    if (srPostAt) {
      await Promise.all(srChannels.map(ch => client.chat.scheduleMessage({ channel: ch, text: report, post_at: srPostAt, mrkdwn: true })));
      const scheduledTime = new Date(srPostAt * 1000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
      await Promise.all(srChannels.map(ch => client.chat.postMessage({ channel: ch, text: `⏰ *Shift Handoff scheduled for ${scheduledTime}*`, mrkdwn: true })));
    } else {
      await Promise.all(srChannels.map(ch => client.chat.postMessage({ channel: ch, text: report, mrkdwn: true })));
    }
    extractAndUpdateVocab(allNotes);
  } catch (err) {
    console.error('[/shiftreport] Error:', err);
    await Promise.all(srChannels.map(ch => client.chat.postMessage({ channel: ch, text: `❌ Error: ${err.message}` })));
  }
});
